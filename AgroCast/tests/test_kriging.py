# -*- coding: utf-8 -*-
"""Тесты обыкновенного кринга: вариограмма, точность, детренд, IDW-фолбэк."""
import numpy as np
import pytest
...
def test_variogram_shapes():
    ...
    assert exponential_variogram(0.0, 0.2, 0.8, 100.0) == 0.0      # γ(0)=0
    g = np.array([vg.gamma(h) for h in (1.0, 50.0, 200.0, 2000.0)])
    assert np.all(np.diff(g) > 0)                                   # монотонный рост
    assert abs(g[-1] - vg.sill) < 1e-8                              # выход на плато
    assert abs(vg.cov(0.0) - vg.sill) < 1e-12                       # C(0)=sill
    assert abs(vg.cov(2000.0)) < 1e-8                               # C(∞)→0
    # подбор восстанавливает параметры синтетической вариограммы
...
def test_fit_variogram_on_smooth_field():
    """По гладкому полю подбор даёт конечные, положительные параметры."""
    rng = np.random.default_rng(5)
    pts = np.column_stack([rng.uniform(44.5, 46.0, 24), rng.uniform(37.5, 40.0, 24)])
    # поле с пространственной корреляцией (радиус ~80 км) + шум
...
def test_exact_interpolation_at_data_points():
    """Кринг — точный интерполятор: в узлах данных прогноз = значение."""
...
def test_constant_field_zero_variance():
    ...
    res = krige(pts, np.full(4, 7.5), np.array([[45.4, 38.8], [45.9, 40.0]]), detrend=False)
    # константа воспроизводится точно (Σw = 1); дисперсия ОК зависит только
    # от геометрии, поэтому проверяем её неотрицательность и конечность
...
def test_plane_detrend_recovers_trend():
    """Поле = плоскость: с детрендом прогноз почти точен, без — заметно хуже."""
    ...
    vg = Variogram(nugget=0.0, psill=0.01, range_km=50.0)  # остатков нет
    on = krige(pts, plane, targets, variogram=vg, detrend=True)
    off = krige(pts, plane, targets, variogram=vg, detrend=False)
...
def test_idw_fallback_on_duplicates():
    """Дубли точек → вырожденная ковариация → честный фолбэк на IDW."""
    pts = np.array([[45.0, 38.0], [45.0, 38.0], [45.5, 38.6], [46.0, 39.2]])
    vals = np.array([1.0, 3.0, 2.0, 0.5])
    res = krige(pts, vals, np.array([[45.0, 38.0], [45.6, 38.8]]), detrend=False)
    assert (res.method == "idw").all()
    assert res.n_fallback == 2
    assert abs(res.pred[0] - vals[0]) < 1e-9            # IDW точен в узле данных
    # мало точек — тоже фолбэк
    res2 = krige(pts[:2], vals[:2], np.array([[45.2, 38.2]]), detrend=False)
