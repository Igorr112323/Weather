...
PostgreSQL — зелёный. Это инфраструктурный fix T06-контура: научный допуск и приём
...
### Пересчёт пространственного навыка по локальному факту · T10

**Дата:** 2026-09-06 · **Владелец:** ML · **Ветка:** `arena/01a075bf-weather` (не смержена, PR #8 открыт)

- **Общий сервис:** `agrocast/skill/spatial.py` — метрики каждой строки пайплайн-таблицы движка (`rps_rows` против локальных tercile-классов ячейки, `hit=argmax`, coverage физического интервала `[mu+sd·q10, mu+sd·q90]`), RPSS против локального эмпирического терцильного климат-бейзлайна ячейки (частоты по target-месяцу внутри проверочного окна, сглаживание +1).
- **Единый источник фактов:** движок бэктеста (`return_pipeline=True`) теперь пишет в пайплайн-таблицу `mu`/`sd` ячейки и `evaluation`-метку as-of (`replay_revised`/`prospective`); spatial-отчёт наследует режим проверки T08, отдельные формулы для «региональной» метрики удалены из пути расчёта.
- **Зависимости учёта:** time-ряд сортируется по целевым периодам и оценивается moving-block bootstrap (перекрывающиеся блоки длины 12 мес. для monthly и 6 мес. для seasonal — сезонные окна перекрываются наблюдательными месяцами, независимость строк не предполагается); пространственная ковариация — ресемплинг блоками 2×2 ячейки (0.5°), публикуются оба числа блоков и CI по отдельности (`rps_ci95_time`, `rps_ci95_space`).
- **Артефакт:** `grid-skill-v2` с полным набором комбинаций `mode_variable_lead` из заданных стартовых месяцев, секцией `spatial` (метод, бейзлайн, размеры блоков, числа блоков, evaluation) и легаси-ключами `monthly_*`/`seasonal_*`/`by_point`, поэтому `GET /api/skill` и сводка региона (`region_block`) читают новый формат без изменений API; `krai_grid_skill.json` бандла (read-only) намеренно не подменён — научные и release-гейты T06–T09 не тронуты.
- **Скрипт:** `scripts/spatial_skill.py --region krai --years 2020-2024 [--mode both --start-months ... --leads-monthly ...]` — перебирает все ячейки региона из `region_cells`, прогоняет движок для каждой, пишет артефакт; для демонстрации на sandbox прогнан seasonal-режим 2020–2024: 28 ячеек, 280 верификаций, t2m RPSS −0.008 (hit 47%, CI [32%, 65%], coverage 93%) против локального климат-бейзлайна — подтверждён уровень «нет преимущества над локальной климатологией», tp RPSS −0.178 (coverage 56%) остаётся ниже уровня; полная генерация с monthly 1–6 — офлайн-расчёт на весь ряд (часы), результат не заявляется, пока не выполнен.
- **Тесты:** `tests/test_spatial_skill.py` (8): смена локального факта ячейки A (её терцильный класс и obs-величина) меняет obs/RPS/hit/coverage и строки `by_point` только у A (B..F бит-в-бит совпадают); локальный бейзлайн различается у ячеек с разными фактами; все комбинации т2м/tp × leads опубликованы с `n_time_blocks`/`n_spatial_blocks`/размером блока; перекрытые сезоны дают меньше блоков, чем строк, и spatial-блоки укрупняют сетку; artifact round-trip с легаси-ключами и сериализуем в JSON; bootstrap детерминирован по seed и чувствителен к смене seed; интеграция с bundle: пайплайн-таблица движка несёт `mu`/`sd`/`obs_tercile`/`evaluation` ячейки и читается spatial-отчётом.
- **Проверки:** ruff F,E9 чистый; профильные наборы `test_region_api`, `test_readiness`, `test_grid`, `test_pipeline_single`, `test_asof`, `test_ledger` зелёные (57 тестов); полный suite 615 passed / 20 skipped (скипы — только флаги browser/pgserver); CI на коммите `03b459f`: run 34051472632 — 4/4 success (identity-postgresql, audit, browser×2), desktop-builds run 34051469844 — success.

### Единые метрики и независимая калибровка · T11

**Дата:** 2026-09-07 · **Владелец:** ML · **Ветка:** `arena/01a075bf-weather` (PR #8 открыт)

- **Единый модуль:** `backtest/metrics.py` — `tercile_baseline` (эмпирическая терцильная климатология по target-месяцу, prior +1), `rpss(..., months, baseline="empirical")` (отношение средних RPS), `ece` (top-label + classwise, равномерные bins=10 с публикацией границ и counts, пропуск бинов <5 строк), `wilson_interval`, `block_bootstrap_ci` (moving-block, детерминирован по seed), `promotion_decision` + константы `PROMOTION_RULE`/`CONFIDENCE_LOW/MEDIUM`. Из ledger удалена локальная копия ECE, из spatial — локальные baseline/bootstrap; audit_full перешёл на общие `wilson_interval`, `tercile_baseline` и `block_bootstrap_ci`.
- **Согласованные определения (2026-09-06):** бейзлайн — эмпирическая климатология (равномерный 1/3 оставлен явной опцией `baseline="uniform"` для исторической сопоставимости); порог продвижения — `RPSS > 0` и нижняя граница 95% block-bootstrap CI > 0. Оба решения зафиксированы в METRICS.md до пересчёта итоговых чисел.
- **Conformal:** квантили поправок берутся с конечновыборочной консервативной поправкой (`floor((n+1)L)−1` / `ceil((n+1)L)−1`); артефакт хранит `fit_years`; `coverage80` отказывается считать покрытие на строках fit-окна (`require_out_of_fit`), публикует Wilson-95, среднюю ширину и явную оговорку об условности гарантии; формулировки «≥80% гарантировано» удалены. TercileCalibrator через `gated_calibrator` также несёт `fit_years` (hold-out-гейт T06 сохранён).
- **Пространственный отчёт (grid-skill-v2):** к combos добавлены `rpss_ci95_time`, `skill_promoted` + `promotion_rule`, `ece_top_label/classwise/bins`, `width80_z`, `fit_exclusion` (число строк, исключённых как попавших в fit-окна conformal/p-calib артефактов; для legacy-артефактов бандла без отпечатка — честная отметка `source="none"`, coverage таких строк не участвует в продвижении). Легаси-ключи дополнены `ece`, `skill_promoted`, `width80_z`; сводка региона выводит строку статуса продвижения.
- **Гейт mini-аудита:** переведён в гейт согласованности (`promotion_contradiction`): CI краснеет, если артефакт навыка заявляет подтверждённый сезонный t2m, а правило продвижения в независимом аудите не выполнено, либо артефакт нечитаем; жёсткие проверки целостности (объём выборок, покрытие 0.72–0.85, ECE≤0.10, hit-пол) сохранены. Порог не понижался: прежний «RPSS>0.05» против равномерного климата был мерой другого (более мягкого) определения.
- **Честный пересчёт на mini5 (5 точек × 20 лет, LOYO, эмпирический бейзлайн):** seasonal t2m RPSS −0.069 (95% CI [−0.124, −0.009]) — не подтверждён; seasonal tp +0.027 (CI [−0.001, +0.053]) — не подтверждён; overall −0.019; покрытие конформированного интервала 0.777 (сырое 0.783). Витрина и API корректно получают `no_skill` (smap пересчитан общим определением; LIVE==engine в тесте T09 сохранён). Навык против равномерной климатологии по-прежнему публикуется в истории, но продвижения не заявляет.
- **Тесты:** `tests/test_metrics_t11.py` (12, включая ловушку противоречия в гейте mini-аудита): идеально калиброванный контроль-пример (n=60k, исходы из собственных P) даёт |ECE|<0.015 и **одинаковые числа в engine-putи, spatial-артефакте, ledger-блоке и audit-group_metrics**; RPSS идеального климат-прогноза = 0; расхождение эмпирического/равномерного бейзлайнов на скошенных данных; Wilson против независимой реализации; таблица правила продвижения; консервативность конечновыборочной поправки; guard in-sample coverage и round-trip `fit_years`; публикация promo/ECE/ширины в combos; механика fit-исключений (18 из 24 строк toy-окна попадают в fit-окно артефакта).
- **Проверки:** ruff F,E9 чистый; полный suite 627 passed / 20 skipped (скипы — только флаги browser/pgserver); mini-аудит 5 точек на sandbox: вердикт PASS (целостность + гейт согласованности; отрицательный навык опубликован как результат, без заявления).

### Безопасные агрорекомендации и единицы · T12

**Дата:** 2026-09-07 · **Владелец:** ML + агроном (сверка в процессе) · **Ветка:** `arena/01a075bf-weather` (PR #8 открыт)

- **Единицы:** `agro/units.py` — единственный переводчик мм↔м³/га (1 мм = 10 м³/га, отрицательный дефицит→0, None/NaN→None); применён в insight (водобаланс, декады), advisor (irrigation_hint), prices (стоимость полива 3 ₽/м³ при 150 мм = 1500 м³/га = 4500 ₽ — перепроверено, расхождения нет). Табличный тест границ — `tests/test_t12_agro.py`.
- **Исправлены дефекты:** (1) в `insight.season_insight` сухой сценарий полива вычитал почвенный кредит в мм из значения в м³/га (смешение единиц; завышал норму на 10×кредит); (2) **запас корнеобитаемого слоя из ERA5 swvl1 (0–7 см) с множителем 2000 мм завышал доступную влагу ~в 20 раз** — функция исключена из расчёта (спорная при отсутствии многослойных данных): публикуются только θ-индикатор поверхности и пояснение, кредит = 0, норма полива = полный дефицит водобаланса (консервативно); (3) чинится молча падавший блок top_drivers (NameError stds из рефакторинга T09 — теперь ctx["stds"], except больше не скрывает отсутствие драйверов).
- **Политика рекомендаций** `agro/policy.py`: статус переменной берётся из `skill_promoted` артефакта `grid-skill-v2` (легаси-артефакт без поля → unconfirmed; нет/битый файл → unavailable). `tag_agro` навешивает статус на water/drought/frost/econ/decisions; карточки `_what_to_do`: при confirmed — «Запланировать…» (high), при unconfirmed — «Ориентировочно (не предписание): …» (info) с причиной, при unavailable — карточка не формируется, irrigation_hint удаляется из payload. Таблица решений decide и verdict'ы econ переформулированы так же; phenology-действия фильтруются по переменной риска (drought/увлажнение→tp, жара/заморозки→t2m).
- **Деньги:** econ_block добавляет `money_note` «сценарная оценка… не гарантия дохода», source-строка то же; verdict окупаемости — «окупается (по сценарной оценке)»; digest поля: строка воды — «Полив (ориентировочно: навык tp не подтверждён — не предписание)…», сухой сценарий подписан формулой «(ET0 p90 / осадки p10)», деньги — «(сценарная оценка, не гарантия дохода)»; при unavailable линия полива отсутствует.
- **Привязка поля к точке:** интеграционный тест на реальном bundle — payload forecast_point содержит точные lat/lon запроса поля и grid_point в пределах ячейки 0.5°; сезонная дезагрегация (декады) публикует суммарный p50 и ratio к сезонной медиане с пояснением «квантиль суммы ≠ сумма квантилей».
- **Экспертная сверка:** передан `docs/production/AGRO_CHECKLIST.md` (Kc, САТ, окна сева, пороги заморозка, отраслевые ₽-оценки, пороги решений, правила исключения). Статус: **ожидает заполнения агрономом** — агрорекомендации не объявляются верифицированными вручную до этого; исключённые функции (профильный запас влаги) в релиз не входят.
- **Тесты:** `tests/test_t12_agro.py` (9): таблица единиц и обратный перевод; advisor-подсказка = дефицит×10 и «нет подсказки при малом дефиците»; water без профильного кредита и с корректным сухим сценарием; матрица статусов политики (v2 confirmed/unconfirmed, легаси, отсутствующий и битый артефакты); tag_agro+карточки (не предписание / подавление); decision_table; money_note и источник «не гарантия дохода»; строки digest; привязка поля–точки на bundle.
- **Проверки:** ruff F,E9 чистый на изменённых файлах (в orchestrator попутно удалены мёртвые импорты, оставшиеся после миграции T09; в drivers/phenology пред-существующие F401 не трогали — вне диффа); полный suite: 636 passed / 20 skipped за 5:56 локально; CI green на обоих коммитах этапа — код `e9adbf8` и docs `4499686` (audit = pytest-сьют + мини-аудит mini5, identity-postgresql, browser×2); PR #8 (OPEN) head = `8f787aa` (docs-уточнение поверх 4499686; CI green на всех трёх коммитах этапа), не мержится.

### Локальные релизы данных и обновляемый инжест · T14 + T13 (локальный контур)

**Дата:** 2026-09-07 · **Владелец:** BE + ML · **Ветка:** `arena/01a075bf-weather` (PR #8 открыт)

- **T14 — bundle release-механизм** (`agrocast/bundle/releases.py`, `scripts/bundle_publish.py`): контент-адресные релизы (sha256 на файл + digest манифеста), идемпотентная пересборка, активация только после полной верификации, атомарный `active.json` с историей и откатом через ближайший предшествующий валидный release; corrupt-кандидаты пропускаются.
- **Интеграция десктопа:** каждый запрос через `use_active_bundle` (битый release — явная ошибка, не fallback); идентичность результата и ключ кэша считают `data_release` по digest `release.json` активного bundle — смена версии не отдаёт устаревший кэш; `LocalInputsResponse` публикует `bundle_release`; readiness (веб) добавляет `bundle_release` (corrupt → not_ready); запуск приложения останавливается на повреждённом активном release с подсказкой rollback.
- **ZarrStore.write**: безопасный swap (tmp → backup → rename, откат при сбое rename) — прерванная запись не оставляет пустой источник и не портит предыдущий снимок; `.old` подчищается.
- **T13 локальный контур**: `drop_incomplete_months` (месяц пишется только при полной доступности дней и ≤25 % NaN в строке; иначе событие в реестр), идемпотентный append с time-dedup, `scripts/refresh_sources.py` — единая точка входа обновления с межплатформенной файловой блокировкой (повторный запуск → код 4). Остаток T13 (CDS-DAG, расписание, SLA источников веб-контура) — в backlog.
- **Тесты:** `tests/test_bundle_release_t14.py` (9): build/verify/idempotency/activate/rollback (в т. ч. пропуск повреждённых), reject corrupt при активации, отсутствие staging-осколков, конкурентный читатель видит только целые release-ы, идентичность следует версии бандла, swap ZarrStore при сбое записи, readiness-блок. `tests/test_source_refresh_t13.py` (4): таблица границ месяцев (включая февраль/високосный/25 % порог), grid-NaN, идемпотентный append, CLI-аргументы и блокировка.
- **Проверки:** ruff F,E9 чистый на изменённых файлах; смежные наборы (test_desktop 9, runtime settings/contracts 155, smoke/pipeline/region 25) зелёные; полный suite и CI — по итогам пуша.
- **Не объявляется готовым:** подпись/доверие источника релиза (T15), серверный расписание-обновитель, фактическая выкладка установщиков в каталог загрузок.

### Права на данные, приватность, экспорт/удаление аккаунта · T23 (техническая часть)

**Дата:** 2026-09-07 · **Владелец:** PO + Ops + юрист (юр. часть не завершена) · **Ветка:** `arena/01a075bf-weather` (PR #8 открыт)

- HTTP-контур: `GET /api/account/export` — полный экспорт своих fields/crops/subscriptions/jobs/publications без хэшей паролей; `DELETE /api/account` — безвозвратное удаление только после повторного ввода пароля, с 409 на незавершённых задачах, каскадом ресурсов/сессий и удалением осиротевшей организации. Политики допуска в `security.py` обновлены (все аутентифицированные роли; CSRF/Origin обязательны).
- Инвентарь `docs/production/DATA_LICENSES.md`: источник → условия → способ поставки (машина пользователя качает NOAA/CPC напрямую; в поставке — производный seed-bundle и атрибуции), приватность десктопа (state_dir/localStorage, телеметрии нет), статусы: публично подтверждённые условия отдельно от **ожидающих юриста** (Copernicus-формулировка атрибуции, лицензия кода — её нет, условия использования — текст за PO).
- Тесты: `tests/test_account_privacy_t23.py` (2) + смежные наборы identity/контрактов (289) зелёные.
- Не объявляется готовым: юридическая верификация, выбор лицензии кода, утверждённый текст «что продукт не обещает» (часть T24).

### Бюджет производительности и канал артефактов · T22 (замер) + T20 (поставка)

**Дата:** 2026-09-07 · **Владелец:** BE + Ops · **Ветка:** `arena/01a075bf-weather` (PR #8 открыт)

- **Замер (T22):** `scripts/perf_report.py` воспроизводит числа на любом стенде: точка seasonal cold 10.8 с / warm-кэш ~0 с, peak RSS 831 МБ; поле 28 ячеек на 2 слотах 296 с (стенд-песочница 2 CPU; SLO раздела 6 остаются неутверждёнными — см. PERFORMANCE.md).
- **Найденный и исправленный дефект:** целочисленные ключи месяцев в frost-блоке ломали идемпотентность канонического JSON (числовая сортировка при записи против лексикографической при чтении) — конверт кэша сам себе не совпадал, тёплые запросы всегда пересчитывались. `canonical_json` теперь нормализует ключи к строкам и отклоняет коллизии нормализации; frost публикует строковые ключи. Тесты: round-trip идемпотентность + сквозной hit кэша + регрессия frost.
- **Канал артефактов (T20-поставка):** job `release` в `desktop-builds`: по тегу `desktop-v*` собирает три ОС с desktop-тестами и смоуком на том же коммите, пакует zip/tar, считает `SHA256SUMS`, создаёт draft GitHub Release с честной пометкой о неподписанности и откате данных через `bundle_publish rollback`. Фактическая публикация не выполнялась (квота Actions исчерпана 07.09) и не объявляется готовой.
- **Проверки:** после фикса — полный suite 651 passed / 20 skipped зелёный; warm-hit подтверждён живым прогоном (0.0 с); ruff F,E9 чистый.
...
### Политика полноты фактов при верификации · T16 (реализовано, 2026-09)

- Мотивация: проверка фактов в `run_backtest` брала месячный `z` из агрегатов без политики полноты; частичные месяцы и all-NaN осадки (`sum()` по NaN-дням) молча попадали в оценку, а NaN-факт сравнениями превращался в верхний терциль.
- Реализация: `agrocast/core/observation.py` — `month_completeness(daily)` требует ровно `days_in_month` календарных дней и ненулевых значений по каждому оцениваемому переменной (сезонное окно проверяется по всем месяцам; провал окна = `fact_window_missing`/`fact_incomplete`/`fact_not_finite`). Движок верификации пропускает неверифицируемые цели, пишет счётчик pending в событийный реестр, каждая запись получает `obs_revision` — 16-символьный fingerprint покрытия и контрольной суммы фактов.
- Агрегация: `monthly_from_daily` для осадков считает сумму с `min_count=1` — полностью пропущенный месяц теперь NaN (исключается политикой), а не климатический ноль; частичные пропуски видны только через completeness policy (на отображение не влияют).
- Приёмка: сезон оценивается только после полных месяцев; два идентичных прогона не дублируют записи (уникальный ключ variable×start_month×lead×year×model×target_start проверен фикстурой); изменение хвоста наблюдений меняет `obs_revision` при неизменных прогнозах; фикстуры покрывают пропуски дней, 29-дневный февраль, переход года через декабрь/январь. Тесты: `tests/test_observation_t16.py` (10), регрессия as-of поprediction обновлена вычитанием метаданной колонки.
- Не закрыто этой задачей: сверка калибровки станционных фактов с сеточными (внешний оператор), ревизия прогнозов (T15), продуктовый UI pending-состояний (T21).

### Надёжный согласованный UI десктопного контура · T21 (реализовано, 2026-09)
