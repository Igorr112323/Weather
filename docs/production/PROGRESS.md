...
Проверено CI (`desktop-builds`): ubuntu и macos полностью зелёные — тесты, смоук, сборка
PyInstaller и загрузка артефактов; Windows — два найденных и исправленных дефекта
переносимости (directory fsync, cp1251), финальный зелёный прогон ещё не подтверждён
(доступ к API CI прервался). Подробности и границы — `DESKTOP.md`.
